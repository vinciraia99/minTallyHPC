...
unsigned long long simulate(Parameters *params, Tally *tallies, sycl::queue &q)
{
  try
  {
    unsigned long long *verification = sycl::malloc_shared<unsigned long long>(1, q);
    *verification = 0;

    // Loop over particles
    q.parallel_for(sycl::range<1>(params->n_particles),
                   sycl::reduction(verification, std::plus<unsigned long long>()),
                   [=](sycl::item<1> idx, auto &verification_reduction)
                   {
                     // Get the RNG seed
                     uint64_t seed = init_seed(idx[0] + 1, params->seed);
